...
class BackupController extends Controller
{
    public function index(){
        ...
        $files = $disk->files('/MUBAKID/');
        $backups = [];
        foreach ($files as $k => $f) {
           if (substr($f, -4) == '.zip' && $disk->exists($f)) {
               $backups[] = [
               'file_path' => $f,
               'file_name' => str_replace(config('laravel-backup.backup.name') . 'MUBAKID/', '', $f),
    ...
    public function download($file_name) {
        $file = config('laravel-backup.backup.name') .'/MUBAKID/'. $file_name;
        $disk = Storage::disk(config('laravel-backup.backup.destination.disks'));

        if ($disk->exists($file)) {
            $fs = Storage::disk(config('laravel-backup.backup.destination.disks'))->getDriver();
            $stream = $fs->readStream($file);
            return response()->download(storage_path('/app/MUBAKID/'. $file_name));
     ...
     public function delete($file_name){
          $disk = Storage::disk(config('laravel-backup.backup.destination.disks'));
          if ($disk->exists(config('laravel-backup.backup.name') . '/MUBAKID/' . $file_name)) {
               $disk->delete(config('laravel-backup.backup.name') . '/MUBAKID/' . $file_name);
